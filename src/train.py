...
def _parse_args():
    # Load the YAML configuration from config.yaml
    with open('config.yaml', 'r') as config_file:
        config = yaml.safe_load(config_file)

    # Create an empty Namespace object to hold the arguments
    args = argparse.Namespace()

    # Iterate through the loaded config dictionary and set attributes in the args Namespace
...
def main():
    ...
    print(f'Training with a single process on %d GPUs.' % args.num_gpu)
    ...
    if not os.path.isdir(eval_dir):
        ...
        if not os.path.isdir(eval_dir):
            print('Validation folder does not exist at: {}'.format(eval_dir))
            exit(1)
